...
class item_location::item_on_map : public item_location::impl
{
    ...
    public:
        ...
        int obtain_cost( const Character &ch, long /* qty */ ) const override {
            ...
            int mv = dynamic_cast<const player *>( &ch )->item_handling_cost( *what );
    ...
    public:
        ...
        int obtain_cost( const Character &ch, long /* qty */ ) const override {
            ...
            if( holster && who.is_worn( *holster ) && holster->can_holster( *what, true ) ) {
                ...
                mv += dynamic_cast<const player &>( who ).item_handling_cost( *what );
                for( auto obj = who.find_parent( *what ); obj &&
                     who.find_parent( *obj ); obj = who.find_parent( *obj ) ) {
                    mv += dynamic_cast<const player &>( who ).item_handling_cost( *obj );
                }
            }
